...
import planeFragment from "../shaders/plane-instancing.fragment.glsl";
import planeVertex from "../shaders/plane-instancing.vertex.glsl";

export default function Instancing() {
  const textures = useTexture(imagesData.map((image) => image.image));

  const instanceCount = textures.length * 10;
  const instancedBufferGeoRef = useRef<THREE.InstancedBufferGeometry>(null);
  const PlaneGeometry = useMemo(
    () => new THREE.PlaneGeometry(1.78, 1, 32, 32),
    []
  );
  ...
  const { positionsArr, rotationArr, flipRotationArr, colorArr } =
    useMemo(() => {
      ...
      for (let i = 0; i < instanceCount; i++) {
        ...
        if (
          (planeGroupIndex > 2 && planeGroupIndex < elementsPerCircle / 2) ||
  ...
  const uniforms = useMemo(() => {
    return {
      ...
      texture5: { value: textures[4] },
      texture6: { value: textures[5] },
      texture7: { value: textures[6] },
      texture8: { value: textures[7] },
      texture9: { value: textures[8] },
      texture10: { value: textures[9] },
      texture11: { value: textures[10] },
    };
  }, [textures]);
